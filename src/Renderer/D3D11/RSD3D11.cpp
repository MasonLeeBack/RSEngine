...
namespace rs::Render {
    bool RSD3D11::Initialize() {
        ...
        ImGui_ImplWin32_Init(renderHwnd);
    ...
    RSRender_Shader* RSD3D11::CreateShaderFromFile(RSShaderStage stage, const char* fileName) {
        ...
        l_DeviceContext->IASetPrimitiveTopology(conv.getTopology(RS_PRIMITIVE_TOPOLOGY_TRIANGLELIST));
        RSRender_Shader* shader = new RSD3D11_Shader(desc);
    ...
    void RSD3D11::SetShader(RSRender_Shader* shader) {
        ...
        switch (stage) {
        case RS_SHADER_STAGE_VS:
            vertexShader = (ID3D11VertexShader*)shader->getShader();
            l_DeviceContext->VSSetShader(vertexShader, NULL, NULL);
            break;
        case RS_SHADER_STAGE_PS:
            pixelShader = (ID3D11PixelShader*)shader->getShader();
            l_DeviceContext->PSSetShader(pixelShader, NULL, NULL);
            break;
        }
    }
    ...
    void RSD3D11::SetVertexBuffer(RSRender_Buffer* buf) {
        ...
        ID3D11Buffer* buffer = (ID3D11Buffer*)buf->getBuffer();
        if (desc.id == RS_VERTEX_BUFFER) {
            l_DeviceContext->IASetVertexBuffers(0, 1, &buffer, &desc.uStride, &desc.uOffset);
        }
    }
    ...
    void RSD3D11::SetIndexBuffer(RSRender_Buffer* buf) {
        ...
        if (desc.id == RS_INDEX_BUFFER) {
            l_DeviceContext->IASetIndexBuffer(buffer, DXGI_FORMAT_R32_UINT, 0);
        }
    }
