...
namespace rs {
	...
	CEditor::CEditor(RSEngine* EngineInstance) : m_Engine(EngineInstance)
	{
	}
	...
	bool CEditor::Initialize() {
		...
		io.IniFilename = NULL;
		fonts.push_back(io.Fonts->AddFontFromFileTTF("data/fonts/Roboto-Medium.ttf", 16.0f));
	...
	bool showNewInstance(bool* p_open) {
		...
		if (ImGui::Begin("New Instance...", p_open, ImGuiWindowFlags_NoResize)) {
			...
			if (ImGui::Button("OK", ImVec2(89, 0))) {
				*p_open = false;
				NewInstance(myPart, Part);
				myPart->Size = Vector3(1, 1, 1);
			}
